...
ADDRESS_INFO_PATH = TRACKER_DB_DIR / "addrs_backup.json"
USERS_INFO_PATH=TRACKER_DB_DIR / "users_backup.json"
class Tracker_Backup:
    def __init__(self):
        ...
        self.nodes_info_path = TRACKER_DB_DIR / "nodes_backup.json"
        self.files_info_path = TRACKER_DB_DIR /  "files_backup.json"
    ...
    def ensure_directories(self):
        ...
        try:
            ...
            TRACKER_DB_DIR.mkdir(parents=True, exist_ok=True)
            
            # Set quyền cho thư mục (chỉ cần thiết trên Linux)
            if os.name != 'nt':  # Không phải Windows
                os.chmod(TRACKER_DB_DIR, 0o777)
                
            logging.info(f"Directory structure verified: {TRACKER_DB_DIR}")
        except Exception as e:
            logging.error(f"Failed to create/verify directories: {e}")
            raise
    ...
    def save_users(self):
        ...
        try:
            # Đảm bảo thư mục tồn tại với đúng quyền
            self.ensure_directories()
            
            # Convert Path to string when opening file
            with open(str(USERS_INFO_PATH), 'w') as f:
                json.dump(self.users, f, indent=4)
                
            # Set file permissions (chỉ cần thiết trên Linux)
            if os.name != 'nt':
